...
export default function Mens() {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.post('http://192.168.0.106:3000/men-data', {seller :userData.userId });
  ...
  const handleUpdateProduct = async () => {
    ...
    try {
      ...
      const response = await axios.put('http://192.168.0.106:3000/men', payload);
  ...
  const handleDeleteProduct = async(productId) => {
    try {
      ...
      const res = await axios.delete('http://192.168.0.106:3000/men', {
        data: { id: productId } 
      });
      if (res.data) {
        alert("Deleted");
  ...
  const handleAddProduct = async() => {
    try {
      ...
      const response = await axios.post('http://192.168.0.106:3000/men', formData, {
        headers: {
